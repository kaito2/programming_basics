...
let select_even = List.filter (fun n -> n mod 2 = 0)
...
let count_A students = List.length (List.filter (fun student -> student.rank = "A") students)
...
let cocat strings = List.fold_right (fun s1 s2 -> s1 ^ s2) strings ""
...
let student_sum students = List.fold_right (fun st sum -> sum + st.score) students 0
