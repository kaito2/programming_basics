...
(* 目的: ekimei_t のリストと始発の漢字駅名を受け取り、初期化された station_t のリストを返す *)
(* make_initial_station_list : string -> ekimei_t list -> station_t list *)
let make_initial_station_list start_station_kanji ekimei_lst = 
    List.map
        (fun ekimei -> if ekimei.kanji = start_station_kanji
            then {name=ekimei.kanji; shortest_distance_meter=0.; path=[ekimei.kanji]}
            else {name=ekimei.kanji; shortest_distance_meter=infinity; path=[]})
        ekimei_lst
